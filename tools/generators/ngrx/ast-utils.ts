...
export function addImportToTestBed(
  ...
  const configureTestingModuleObjectLiterals = allCalls
    ...
    .map(c =>
      c.arguments[0].kind === ts.SyntaxKind.ObjectLiteralExpression ? c.arguments[0] : null
    );
  ...
  if (configureTestingModuleObjectLiterals.length > 0) {
    ...
    return insertChange(host, source, specPath, startPosition, `imports: [${symbolName}], `);
  }
  return source;
}
...
export function replaceIntoToTestBed(
  ...
  if (configureTestingModuleObjectLiterals.length > 0) {
    const startPosition = configureTestingModuleObjectLiterals[0].getFirstToken(source).getEnd();
    return replaceChange(host, source, specPath, startPosition, newSymbol, previousSymbol);
  }
  return source;
}
  ...
  if (imports?.initializer.kind === ts.SyntaxKind.ArrayLiteralExpression) {
    ...
    for (const e of a.elements) {
      if (e.kind === ts.SyntaxKind.CallExpression) {
        ...
        if (
          (text === 'RouterModule.forRoot' || text === 'RouterModule.forChild') &&
          ...
          if (routes.kind === ts.SyntaxKind.ArrayLiteralExpression) {
            ...
            if (routesDeclaration) {
              return (routesDeclaration.initializer as ts.ArrayLiteralExpression).elements;
            }
          }
        }
      }
...
export function readBootstrapInfo(
  ...
  const bootstrapComponentFileName = `./${path.join(
    ...
    `${
      names(bootstrapComponentClassName.substring(0, bootstrapComponentClassName.length - 9))
        .fileName
    }.component`
  )}`;
