...
export function addImportsToModule(tree: Tree, options: NgRxGeneratorOptions): void {
  const modulePath = options.module;
  const sourceText = tree.read(modulePath, 'utf-8');
  let sourceFile = createSourceFile(modulePath, sourceText, ScriptTarget.Latest, true);
  const addImport = (
    source: SourceFile,
    symbolName: string,
    fileName: string,
    ...
    return insertImport(tree, source, modulePath, symbolName, fileName, isDefault);
  };
  ...
  if (options.minimal && options.root) {
    ...
    sourceFile = addImportToModule(tree, sourceFile, modulePath, effectsForEmptyRoot);
    ...
    if (hasRouter) {
      ...
      sourceFile = addImportToModule(tree, sourceFile, modulePath, storeRouterModule);
    }
  } else {
    const addCommonImports = (): SourceFile => {
      ...
      if (options.facade) {
        ...
        sourceFile = addProviderToModule(tree, sourceFile, modulePath, facadeName);
      }
    ...
    if (options.root) {
      ...
      sourceFile = addImportToModule(tree, sourceFile, modulePath, storeForRoot);
      sourceFile = addImportToModule(tree, sourceFile, modulePath, effectsForRoot);
      sourceFile = addImportToModule(tree, sourceFile, modulePath, devTools);
      ...
      sourceFile = addImportToModule(tree, sourceFile, modulePath, storeForFeature);
    } else {
      sourceFile = addCommonImports();

      sourceFile = addImportToModule(tree, sourceFile, modulePath, storeForFeature);
      sourceFile = addImportToModule(tree, sourceFile, modulePath, effectsForFeature);
    }
  }
}
