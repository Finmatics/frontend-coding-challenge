 ...
 */
export function addExportsToBarrel(tree: Tree, options: NgRxGeneratorOptions): void {
  // Don't update the public barrel for the root state, only for feature states
  if (options.root) {
    return;
  }

  const indexFilePath = joinPathFragments(dirname(options.module), '..', 'index.ts');
  if (!tree.exists(indexFilePath)) {
    return;
  }

  const indexSourceText = tree.read(indexFilePath, 'utf-8');
  let sourceFile = createSourceFile(indexFilePath, indexSourceText, ScriptTarget.Latest, true);
